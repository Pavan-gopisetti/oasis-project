    ...
    public static void main(String[] args) {
        ...
        for (int i = startingNumber; i <= Attempts; i++) {
            ...
            String number = JOptionPane
                    .showInputDialog("Guess the number between " + startingNumber + " and " + endingNumber + ":");
            int guessNumber = Integer.parseInt(number);

            if (guessNumber == randomNumber) {
                JOptionPane.showMessageDialog(null, "Congratulations!!!! You've guessed the right number : "
                        + guessNumber + " in " + i + " attempts.");
                score = Attempts - i + 1;
                break;
            } else if (guessNumber < randomNumber) {
                JOptionPane.showMessageDialog(null, "Your guess number is lower than random number. Try again. "
                        + (Attempts - i) + " Attempts Left.");
            } else {
